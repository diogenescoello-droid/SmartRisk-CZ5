...
ok(rollout.includes('11.0.0-rc17-operativo-p0') && index.includes('v11-rollout.js?v=11.0.0-rc17-operativo-p0'), 'Versión de caché operativa consistente');
...
ok(!/\.collection\([^)]*\)\.(add|set|update|delete)\s*\(/.test(dashboard), 'RC8 no introduce escrituras Firestore directas');
ok(!/\.doc\([^)]*\)\.(set|update|delete)\s*\(/.test(dashboard), 'RC8 conserva separación entre dashboard y repositorio');

console.log('\nTodas las pruebas consolidadas RC8 pasaron sobre P0.');
