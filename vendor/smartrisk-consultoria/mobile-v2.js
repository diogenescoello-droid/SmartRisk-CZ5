(() => {
  ...
  function injectStyles(){
    ...
    const s=document.createElement('style');s.id='srMobileV2Styles';s.textContent=`
      ...
      @media(max-width:780px){
        ...
        .sr-help{display:none!important}.mobile-more-sheet{grid-template-columns:1fr 1fr!important}
        #mobileGuideButton{display:block}
        .kpis{scroll-padding-left:1px}.kpi{min-width:42vw}.kpi strong{font-size:20px}
      }
    `;document.head.appendChild(s);
  }
  function simplifyButtons(){if(!mq.matches)return;const a=document.getElementById('showCriteria'),b=document.getElementById('marketNewProject');if(a)a.textContent='Criterios';if(b)b.textContent='+ Oportunidad'}
  ...
  function watch(){const obs=new MutationObserver(()=>refresh());obs.observe(document.body,{childList:true,subtree:true});mq.addEventListener?.('change',refresh)}
