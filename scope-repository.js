(() => {
  ...
  function emptyData(){return{territorios:[],instituciones:[],usuarios:[],sitios:[],acciones:[],seguimientos:[],entidadesSeguimiento:[],decisiones:[],validaciones:[],auditoria:[],actoresCOE:[],equiposCOE:[],actividadesCOE:[],capasGeograficas:[],sesionesCabina:[],tareasCabina:[],cartografiaOperativa:[],fichasTecnicas:[],_revision:0}}
  function normalizeType(value){
    const key=String(value||"").trim().toLowerCase();
    return({territorio:"territorios",institucion:"instituciones",usuario:"usuarios",sitio:"sitios",accion:"acciones",decision:"decisiones",validacion:"validaciones",actorcoe:"actoresCOE",equipocoe:"equiposCOE",actividadcoe:"actividadesCOE",capageografica:"capasGeograficas",sesioncabina:"sesionesCabina",tareacabina:"tareasCabina",cartografiaoperativa:"cartografiaOperativa",fichatecnica:"fichasTecnicas"})[key]||key;
  }
  ...
  async function loadRecords(db,keys){
    ...
    for(const key of keys){
      ...
      }catch(error){console.warn(`No fue posible consultar el alcance ${key}`,error)}
    }
    return[...records.values()];
  }
  function snapshot(data,reference){return{exists:true,id:"datos",ref:reference,metadata:{fromCache:false,hasPendingWrites:false},data:()=>clone(data)}}
  function installProfileBridge(db,user){
    ...
    db.collection=function(name){
      ...
      return new Proxy(collection,{get(target,property){
        if(property!=="doc"){const value=target[property];return typeof value==="function"?value.bind(target):value}
        return id=>{
          const reference=target.doc(id);
          if(id!==user.uid)return reference;
          return new Proxy(reference,{get(refTarget,refProperty){
            if(refProperty!=="get"){const value=refTarget[refProperty];return typeof value==="function"?value.bind(refTarget):value}
            return async()=>{
              const snap=await refTarget.get();
              return{exists:snap.exists,id:snap.id,ref:snap.ref,metadata:snap.metadata,data:()=>window.SmartRiskScope.getAppProfile()};
            };
          }});
        };
      }});
  ...
  function installVirtualCloud(db,overlayRef,initialData){
    ...
    const virtualRef={
      __smartRiskVirtual:true,path:CLOUD_DOC,
      get:async()=>snapshot(currentData,virtualRef),
      set:async value=>{
        const filtered=window.SmartRiskScope.filterData(value);
        filtered._scope={type:window.SmartRiskScope.getState()?.scopeType,label:window.SmartRiskScope.scopeLabel(),updatedBy:repositoryState.user.email,updatedAt:new Date().toISOString()};
        await overlayRef.set(filtered);
        currentData=clone(filtered);
      },
      onSnapshot:(success,failure)=>{
        success(snapshot(currentData,virtualRef));
        try{return overlayRef.onSnapshot(snap=>{
          if(!snap.exists)return;
          currentData=window.SmartRiskScope.filterData(mergeData(repositoryState.sourceData,snap.data()));
          success(snapshot(currentData,virtualRef));
        },failure)}catch(error){failure?.(error);return()=>{}}
      }
    };
  ...
  async function init({user,profile,db}){
    ...
    const overlayRef=db.collection("alcances").doc(parentKey).collection("estado").doc(`plataforma-${safeToken}`);
    let overlay={};
    try{const snap=await overlayRef.get();if(snap.exists)overlay=snap.data()}catch(error){console.warn("Estado territorial sin escritura habilitada",error)}
    ...
    return{mode:"territorial-overlay",scopeKey:parentKey,scopeLabel:window.SmartRiskScope.scopeLabel()};
  }
  window.SmartRiskScopeRepository={init,version:"13.2.0",architecture:"unified-ui-scope-overlay"};
})();