(() => {
  ...
  function financingFor(canton, proposalCount) {
    ...
    return { denominator: Math.max(Number(proposalCount) || 0, records.length), withAmount, withSource, total: withAmount.reduce((sum, record) => sum + budget(record), 0), classified, states };
  }
  ...
  function installCockpit() {
    ...
    const render = () => {
      ...
      const review = (window.ENOS_REVIEWS?.reviews || []).find(item => normalize(item.plan) === normalize(context.source));
      const actions = Number(review?.proposalCount || 0), scopeLabel = selected.length === 1 ? gad.gad : `${selected.length} GAD del alcance`, financing = financingFor(review?.territory || canton?.value || gad.gad, actions);
      ...
      cockpit.innerHTML = `<div class="sr-matrix-title"><div><small>LECTURA EJECUTIVA · INFORMACIÓN PRELIMINAR</small><h2>${escapeHtml(scopeLabel)}</h2><p>¿Qué tiene este GAD, qué le falta y cuál es el siguiente paso para validarlo?</p></div><span>NO PUBLICABLE</span></div>
        <div class="sr-matrix-indicators"><article><small>Sitios o lugares identificados</small><b>${context.namedSites.length}</b><span>${context.namedSites.length ? "Revisar identidad y coordenadas" : "Confirmar directamente en el plan"}</span></article><article><small>Acciones propuestas</small><b>${actions}</b><span>${gad.statuses.F04 === "Atribuible" ? "Existen registros atribuibles" : `F04: ${escapeHtml(gad.statuses.F04)}`}</span></article><article><small>Acciones con monto</small><b>${financing.withAmount.length}/${financing.denominator}</b><span>${financing.withAmount.length ? `${money(financing.total)} identificados` : "Sin presupuesto verificable"}</span></article><article><small>Formularios con brecha</small><b>${gaps}/7</b><span>${attributable} atribuibles; aún requieren validación</span></article><article><small>Listos para publicar</small><b>0</b><span>Requieren validación técnica y coordinación</span></article></div>
