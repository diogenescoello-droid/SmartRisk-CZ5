(() => {
  ...
  const VERSION="2026-07-31T13:05:00-05:00";
  let wrapped=false;
  let applying=false;
  let attempts=0;
  ...
  function matchRecord(item,index=locationIndex()){
    ...
    if(canton&&index.cantons.has(canton)){
      return !province||index.provinces.has(province);
    }
  ...
  function reviewEvaluated(review){
    return review?.score!=null||review?.reviewScore!=null||Boolean(review?.criteria?.length);
  }
  ...
  function filterReviews(index){
    ...
    pack.stats={
      ...
      totalChecklist,
      territorialCoverage:percent(received,canonical),
      reviewCompletion:percent(evaluated,received),
      validatedPlans:validated,
      scopeFiltered:true,
      scopeLabel:window.SmartRiskScope?.scopeLabel?.()||"Alcance autorizado"
    };
  }
  ...
  function filterRiskLocations(index){
    ...
    pack.stats={
      ...
      territoriesWithMentions:unique(locations.map(item=>normalize(item?.territory||item?.canton||item?.municipio))).length,
      plansReviewed:Math.max(1,index.rows.length),
      byType,
      byQuality,
      scopeFiltered:true
    };
  ...
  function filterCases(index){
    ...
    ["cases","events","records","locations","summaries"].forEach(key=>{
      if(Array.isArray(pack[key]))pack[key]=pack[key].filter(item=>matchRecord(item,index));
    });
  ...
  function applyGlobals(){
    ...
    if(Array.isArray(window.F03_CARTOGRAPHY)){
      window.F03_CARTOGRAPHY=window.F03_CARTOGRAPHY.filter(item=>matchRecord(item,index));
    }
    if(Array.isArray(window.SMART_RISK_PLAN_UPDATE)){
      window.SMART_RISK_PLAN_UPDATE=window.SMART_RISK_PLAN_UPDATE.filter(item=>matchRecord(item,index));
    }
    if(window.ENOS_IMPORT&&Array.isArray(window.ENOS_IMPORT.sites)){
      window.ENOS_IMPORT.sites=window.ENOS_IMPORT.sites.filter(item=>matchRecord(item,index));
    }
    filterCases(index);
  }
  ...
  function updateVisibleVersion(){
    ...
    document.querySelectorAll(".r023-banner .badge").forEach(badge=>{
      if(badge.textContent!==release)badge.textContent=release;
    });
    document.querySelectorAll("header .badge, header .pill").forEach(element=>{
      if(/RC14\.4\.4\s+RC\d+/i.test(element.textContent||"")){
        element.textContent=String(element.textContent).replace(/RC14\.4\.4\s+RC\d+/ig,release);
      }
    });
  ...
  window.SMART_RISK_TERRITORIAL_SCOPE_GUARD=Object.freeze({
    version:VERSION,
    release:RELEASE,
    apply:enforce,
    matchRecord
