(() => {
  ...
  function validateUser(raw, index) {
    ...
    return { index, raw, correo, nombre, rol, provincia, canton, nivelAcceso, scopeKeys, scopeType, issues };
  }
  ...
  function payloadFor(user) {
    ...
    return {
      ...
      metodoActivacion: "Recuperación por correo",
  ...
  async function provisionOne(user) {
    ...
    if (profiles.length === 1) {
      ...
      return { correo: user.correo, nombre: user.nombre, uid, status: "existing", detail: "Perfil existente actualizado; no se reenvió activación." };
    }
    ...
    let credential;
    try {
      credential = await auxAuth.createUserWithEmailAndPassword(user.correo, makeTemporaryPassword());
    } catch (error) {
      if (error?.code === "auth/email-already-in-use") {
        return { correo: user.correo, nombre: user.nombre, status: "manual", detail: "Authentication ya contiene este correo pero no existe perfil. Se requiere recuperar su UID desde Firebase Authentication." };
      }
      throw error;
    } finally {
    ...
    let activation = "activation-sent";
    try {
      await auth.sendPasswordResetEmail(user.correo);
    } catch (error) {
      console.error("Activation email failed", user.correo, error);
      activation = `activation-error:${error?.code || error?.message || "unknown"}`;
    }
    ...
    return {
      ...
      nombre: user.nombre,
      uid,
      status: activation === "activation-sent" ? "created" : "created-warning",
      detail: activation === "activation-sent"
        ? "Cuenta, perfil y correo de activación creados correctamente."
        : `Cuenta y perfil creados; no se pudo enviar activación (${activation}).`
    };
  }
  ...
  function renderPreview(validated) {
    ...
    table.innerHTML = validated.length ? `<table><thead><tr><th>Usuario</th><th>Rol / alcance</th><th>Validación</th></tr></thead><tbody>${validated.map(item => `<tr><td><b>${escapeHtml(item.nombre || "Sin nombre")}</b><br>${escapeHtml(item.correo || "Sin correo")}</td><td>${escapeHtml(item.rol || "Sin rol")}<br><span class="small">${escapeHtml(item.scopeKeys.join(" · "))}</span></td><td>${item.issues.length ? `<span class="badge bad">${escapeHtml(item.issues.join("; "))}</span>` : '<span class="badge ok">Listo</span>'}</td></tr>`).join("")}</tbody></table>` : "<p>No hay registros cargados.</p>";
  ...
  function renderResults(results) {
    ...
    const warnings = results.filter(item => item.status === "created-warning" || item.status === "manual").length;
    const errors = results.filter(item => item.status === "error").length;
    box.innerHTML = `<div class="status-row"><span class="badge ok">${created} creados</span><span class="badge">${existing} existentes</span><span class="badge ${warnings ? "warn" : "ok"}">${warnings} pendientes</span><span class="badge ${errors ? "bad" : "ok"}">${errors} errores</span></div><div class="table-wrap"><table><thead><tr><th>Usuario</th><th>Resultado</th><th>UID</th></tr></thead><tbody>${results.map(item => `<tr><td><b>${escapeHtml(item.nombre)}</b><br>${escapeHtml(item.correo)}</td><td><span class="badge ${item.status === "created" ? "ok" : item.status === "error" ? "bad" : item.status === "manual" || item.status === "created-warning" ? "warn" : ""}">${escapeHtml(item.detail)}</span></td><td><span class="small">${escapeHtml(item.uid || "—")}</span></td></tr>`).join("")}</tbody></table></div>`;
  }
  ...
  function exportResults() {
    ...
    const safe = {
      ...
      totals: {
        ...
        pending: state.results.filter(item => ["manual", "created-warning"].includes(item.status)).length,
  ...
  function hydrate() {
    ...
    section.innerHTML = `
      ...
      <p class="muted">Carga un JSON local. Los datos permanecen en tu navegador hasta ejecutar el lote. SmartRisk crea Authentication, registra el perfil con su UID y envía recuperación de contraseña a las cuentas nuevas.</p>
      <p class="risk-note"><b>Seguridad:</b> no se muestran ni almacenan contraseñas temporales. Si Authentication ya contiene un correo sin perfil, el registro se detiene para recuperar el UID manualmente.</p>
      <div class="grid">
      ...
      <div class="actions"><button id="runBatchProvision" type="button" disabled>Crear cuentas y activar</button><button id="exportBatchResult" type="button" class="secondary" disabled>Exportar resultado</button></div>
