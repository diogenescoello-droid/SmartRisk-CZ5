...
function validPlanDocumentUrl(value){
 try{const url=new URL(String(value||''));return url.protocol==='https:'?url.href:''}catch{return ''}
}
function reviewOriginalPlanUrl(review){
 const direct=[review?.planFinalUrl,review?.planUrl,review?.documentUrl,review?.sourceUrl,review?.planDraftUrl].map(validPlanDocumentUrl).find(Boolean);
 if(direct)return direct;
 const province=normalizeText(review?.province||''),territory=normalizeText(review?.territory||'');
 const catalog=(window.SMART_RISK_PLAN_SOURCES?.plans||[]).find(item=>normalizeText(item.province)===province&&normalizeText(item.territory)===territory);
 const catalogUrl=validPlanDocumentUrl(catalog?.url);if(catalogUrl)return catalogUrl;
 const entity=(data.entidadesSeguimiento||[]).find(item=>{
  const entityProvince=normalizeText(item.province||item.provincia||''),entityTerritory=normalizeText(item.territory||item.shortName||item.canton||item.name||'');
  return (!province||entityProvince===province)&&territory&&entityTerritory===territory;
 });
 return [entity?.planFinalUrl,entity?.planUrl,entity?.documentUrl,entity?.sourceUrl,entity?.planDraftUrl].map(validPlanDocumentUrl).find(Boolean)||'';
}
function originalPlanButton(review){
 const url=reviewOriginalPlanUrl(review);
 return url?'<button type="button" class="secondary open-original-plan">Abrir PDF original ↗</button>':'<button type="button" class="secondary" disabled title="El enlace verificable del documento original aún no está registrado">PDF original no vinculado</button>';
}
function openOriginalPlan(review){
 const url=reviewOriginalPlanUrl(review);if(!url)return false;
 auditChange('CONSULTAR_PLAN_ORIGINAL','plan',review?.entityId||review?.territory||'',url);save();
 const opened=window.open(url,'_blank','noopener,noreferrer');if(opened)opened.opener=null;return true;
}
window.SmartRiskPlanSource={urlFor:reviewOriginalPlanUrl,buttonFor:originalPlanButton,open:openOriginalPlan};
function openReviewDetail(review){
 ...
 <div class="dialog-actions">${originalPlanButton(review)}<button type="button" class="secondary cancel-bottom">Cerrar</button></div></div>`;
 document.body.append(dialog);dialog.showModal();
 const close=()=>{dialog.close();dialog.remove()};
 dialog.querySelector('.open-original-plan')?.addEventListener('click',()=>openOriginalPlan(review));
...
function actionsPage(){
 ...
 const followups=data.seguimientos||[],linkedFollowups=followups.filter(item=>item.accionId&&item.sitioId).length,pendingHomologation=followups.filter(item=>!item.accionId||!item.sitioId).length;
 ...
 <div class="cards action-cards">
  ...
  <div class="card"><span>Actualizaciones incorporadas</span><strong>${followups.length}</strong><small>${linkedFollowups} vinculadas · ${pendingHomologation} por homologar</small></div>
 </div>
 ...
 $('#addFollowup').onclick=()=>openFollowupForm();
...
function territoryOperationalData(){
 ...
 return (data.territorios||[]).map(territory=>{
  ...
  const followupEntity=(data.entidadesSeguimiento||[]).find(item=>item.territorioId===territory.id);
  const followupPending=Boolean(followupEntity&&normalizeText(followupEntity.baselineStatus).includes('sin remision'));
  const status=!hasPlan?'Sin plan':urgent?'Atención inmediata':followupPending?'Seguimiento pendiente':!sites.length?'Sin fichas gestionables':!actions.length?'Sin acciones':'En gestión';
  const next=!hasPlan?'Solicitar y cargar el plan territorial':urgent?'Responder alertas urgentes y validar evidencia':followupPending?'Registrar la actualización territorial y vincularla con acciones y fichas':!sites.length?'Completar, revisar y aprobar fichas pendientes':!actions.length?'Definir medidas, responsables y plazos':'Controlar avance y evidencia de las acciones';
  return {territory,review,hasPlan,mentions:territoryMentions.length,pending:pending.length,sites,actions,alerts,urgent,followupEntity,status,next};
 });
}
const COE_PROBLEMS={
